...
gsm = SE_Cross.from_options(ICoord1=ic1,nnodes=20,driving_coords=driving_coords,DQMAG_MAX=0.4,BDIST_RATIO=0.9)
...
# => post processing <= #
gsm.icoords[gsm.nR].mol.write("xyz","et_meci.xyz",overwrite=True)

#Save a npz file
Cocc= ls.Tensor.array(gsm.icoords[gsm.nR].PES.lot.lot.casci.reference.tensors['Cocc'])
Cact= ls.Tensor.array(gsm.icoords[gsm.nR].PES.lot.lot.casci.reference.tensors['Cact'])
D   = ls.Tensor.array(gsm.icoords[gsm.nR].PES.lot.lot.casci.reference.tensors['D'])
np.savez(
    'Cmom.npz',
    Cocc=Cocc,
    Cact=Cact,
