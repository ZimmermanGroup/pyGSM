...
class pDynamo(Lot):
    ...
    def __init__(self,options):
        ...
        # ORCA options
        self.set_active('use_orca',False,bool,"Use ORCA to evaluate energies and gradients")
        self.set_active('orca_method',"B3LYP",str,"Method to use in ORCA e.g. HF, MP2, or density functional",
                depend=(self.use_orca),
                msg="Must use ORCA to specify density functional")
        self.set_active('basis',"6-31g",str,"Basis set for wavefunction or density functional",
                depend=(self.use_orca),
                msg="Must use ORCA to specify density functional")
        self.set_active('tole',1e-4,float,"Energy tolerance for convergence")
        self.set_active('maxiter',100,int,"Number of SCF cycles")
        self.set_active('slowconv',False,bool,"Convergence option for ORCA")
        self.set_active('scfconvtol','NormalSCF',str,"Convergence option for ORCA",allowed=['NormalSCF','TightSCF','ExtremeSCF'])
        self.set_active('d3',False,bool,"Use Grimme's D3 dispersion")

        # QM/MM CHARMM 
        self.set_active('qmatom_file',None,str,'')
        self.set_active('use_charmm_qmmm',False,bool,'Use CHARMM molecular mechanics parameters to perform QMMM',
                depend=(self.qmatom_file is not None),
                msg="Must define qm atoms")
        self.set_active('path_to_prm',None,str,'path to folder containing Charmm parameter files')
        self.set_active('path_to_str',None,str,'path to folder containing to Charmm str files')
        self.set_active('psf_file',None,str,'Path to file containing CHARMM PSF')
        self.set_active('crd_file',None,str,'Path to file containing CHARMM CRD')

        # DFTB options
        self.set_active('use_dftb',False,bool,"Use DFTB to evaluate energies and gradients",
                clash=(self.use_orca),
                msg = "We're not using DFTB+")
        self.set_active('path_to_skf',None,str,'path to folder containing skf files')
        self.set_active('use_scc',True,bool,"Use self-consistent charge")

        # General options
        self.set_active('command',None,str,'pDynamo requires a path to an executable like ORCA or DFTB+')
        self.set_active('scratch',None,str,'Folder to store temporary files')

        self.force_active('scratch','scratch/{}'.format(self.node_id),'Setting scratch folder')
    ...
    def get_energy(self,coords,multiplicity,state):
        ...
        return np.asarray(tmp[state][1])  #Kcal/mol
